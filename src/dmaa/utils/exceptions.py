...
class EnvStackNotExistError(Exception):
    def __init__(self,env_stack_name=ENV_STACK_NAME):
        super().__init__( f"Environment stack '{env_stack_name}' does not exist. Please run `dmaa bootstrap` to build the dmaa environment.")
...
class DmaaEnvStackNotReadyError(Exception):
    def __init__(self, env_stack_name=ENV_STACK_NAME):
        super().__init__( f"Environment stack '{env_stack_name}' is not ready. Please run `dmaa bootstrap` to build the dmaa environment.")
